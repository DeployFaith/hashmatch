...
import type { MomentEventRangeMap, ReplayMoment } from "../src/lib/replay/detectMoments.js";
...
const momentRanges: MomentEventRangeMap = new Map([
  ["moment-pre-game", { startEventIdx: 0, endEventIdx: 0 }],
  ["moment-turn-1", { startEventIdx: 1, endEventIdx: 8 }],
  ["moment-turn-2", { startEventIdx: 9, endEventIdx: 16 }],
]);
...
describe("parseCommentaryFile", () => {
  ...
  describe("invalid entries are ignored without throwing", () => {
    ...
    it("handles invalid JSON gracefully", () => {
      const result = parseCommentaryFile("not json at all", moments, EVENT_COUNT, momentRanges);
      expect(result.entries).toHaveLength(0);
      expect(result.warnings).toHaveLength(1);
    ...
    it("handles non-object top level", () => {
      const result = parseCommentaryFile("[1,2,3]", moments, EVENT_COUNT, momentRanges);
      expect(result.entries).toHaveLength(0);
      expect(result.warnings).toHaveLength(1);
    });

    it("handles missing entries array", () => {
      const result = parseCommentaryFile('{"version": 1}', moments, EVENT_COUNT, momentRanges);
      expect(result.entries).toHaveLength(0);
      expect(result.warnings).toHaveLength(1);
  ...
  describe("deterministic ordering", () => {
    ...
    it("produces identical ordering for identical input", () => {
      ...
      const result1 = parseCommentaryFile(text, moments, EVENT_COUNT, momentRanges);
      const result2 = parseCommentaryFile(text, moments, EVENT_COUNT, momentRanges);
...
describe("getVisibleCommentary", () => {
  ...
  it("hides future entries when spoilers are off", () => {
    ...
    const visible = getVisibleCommentary(entries, 5, moments, momentRanges, false);
  ...
  it("shows all entries when spoilers are on", () => {
    const visible = getVisibleCommentary(entries, 5, moments, momentRanges, true);
    expect(visible).toHaveLength(3);
  });

  it("shows entries exactly at the playhead start index", () => {
    // Playhead at index 9 (turn 2 start)
    const visible = getVisibleCommentary(entries, 9, moments, momentRanges, false);
    expect(visible.map((e) => e.id)).toEqual(["c1", "c2"]);
  });

  it("shows all entries at end of replay", () => {
    const visible = getVisibleCommentary(entries, 17, moments, momentRanges, false);
    expect(visible).toHaveLength(3);
  });
});
...
describe("getCommentaryAtIndex", () => {
  ...
  it("returns entries active at a specific event index", () => {
    ...
    const result = getCommentaryAtIndex(entries, 4, moments, momentRanges, 17, true);
  ...
  it("returns no entries when index is outside all ranges", () => {
    ...
    const result = getCommentaryAtIndex(entries, 0, moments, momentRanges, 17, true);
    expect(result).toHaveLength(0);
  });

  it("gates future commentary when spoilers are off", () => {
    // Playhead at index 4, c3 starts at 10 — should be hidden
    const result = getCommentaryAtIndex(entries, 4, moments, momentRanges, 4, false);
    expect(result.map((e) => e.id)).toEqual(["c1", "c2"]);
  });
});
...
describe("getCommentaryForMoment", () => {
  ...
  it("returns entries bound to the moment plus overlapping ranges", () => {
    ...
    const result = getCommentaryForMoment(entries, turn1, moments, momentRanges, 17, true);
  ...
  it("gates future commentary for moment queries", () => {
    ...
    const result = getCommentaryForMoment(entries, turn2, moments, momentRanges, 5, false);
