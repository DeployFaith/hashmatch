...
Per Contract v0 specification (see `Documents/specification.md` §3):
...
Scenarios with hidden state (e.g., a secret number in Number Guess) **must not leak secrets** through mid-game events. Per Contract v0 §9:
...
This section aligns the ecosystem vision with the phased roadmap (see `Documents/roadmap.md`).
