...
|----------|---------------|-------|
| Tournament manifest | `tournament_manifest.json` | Current code writes `tournament.json` — dual-write both for one release, then deprecate `tournament.json` |
| Match manifest | `match_manifest.json` | Does not exist yet — create with this name from the start |
...
**Where to record:** `tournament_rules.md` §8–9 as the single source of truth. Update `tournament_harness_v0.md` §8.1 to match (delete the win=1/loss=0 text). Verify the code's tie-break implementation matches this order.
